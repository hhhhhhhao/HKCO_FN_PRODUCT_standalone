...
def is_title_line(line, title_regex, exclude_regex, sure_regex):
    ...
    if text == '分部之間的銷售乃按照雙方共同協定之條款進行。向執行董事匯報之對外收益,乃按照與綜合':
        print
...
def get_lines_grouped(lines):
    title_regex = [
        ...
        (r"按.*(劃分|分)", 0),
        (r"^(地域資料|分部.*業績|有關.*資料|可呈報.*對賬|管理層.*分析|分部資料|下表.*業績:|股息|流動資金及財務資源|資本架構|資產抵押|業務回顧|分類資料|物業租賃|金融服務|分部報告|可呈報.*業績|財務業績|主要策略性投資|按.*資產|資源投資|地區資料)$", 0),
        (r"^未經.*(表)", 0),
    ...
    sure_regex = [
        ...
        (r"^(其他分部資料:｜綜合全面收益表)$", 0),
        (r"(分部.*如下:)$", 0),
        (r"^[\(（]*[、\)）.．。]*[1234567890一二三四五六七八九十①②③④⑤⑥⑦⑧⑨A-Da-di]+[、\)）.．。]*(來自|分部資料)", 0),
