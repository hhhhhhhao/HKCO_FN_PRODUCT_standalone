...
def _classify_segment_revenue_basis(table, last_period_data=None):
    ...
    真正的上期金额可能因重列、期间变化或单位变化而不同，不能拿来判断本期
    应抽“外部收入”还是“分部收入”。口径必须由当前表明确披露的标签决定。
    """
    patterns = {
    ...
    # 财报产品收入的首选口径是对外客户收入；没有显式对外口径时，依次采用
    # 抵销后的集团/营业收入，再退到报告分部/分部收入。
    priority = ("external", "group", "operating", "report", "segment")
    return next((basis for basis in priority if rows_by_basis[basis]), "external")
...
def _collect_metric_facts(source_tables, revenue_rows):
    ...
    for source in source_tables or []:
        ...
        source_has_leaf_identity = any(
            match_product(cell) in non_total
            for source_row in table if isinstance(source_row, list)
            ...
            if unique_product and not explicit_unique_product:
                ...
                for value, col in numeric_values:
                    ...
                    for product in ([unique_product] + total_names):
                        if (product in total_names and len(non_total) > 1
                                and not source_has_leaf_identity):
                            continue
        ...
        for row in table:
            ...
            if not product:
                continue
            if (product == "合计" and len(non_total) > 1
                    and not source_has_leaf_identity):
                continue
...
def _apply_metric_disclosure_policy(rows, facts, last_period_data=None):
    """孤立总额不能冒充产品级字段；披露许可完全由当前公告事实决定。"""
    product_fact_fields = {
        fact.get("field") for fact in facts or []
        if fact.get("granularity") == "product"
    }
    for field in ("mbcost", "gross_profit"):
        counterpart = "gross_profit" if field == "mbcost" else "mbcost"
        # 本字段显式披露，或当前公告显式披露了等式中的另一个字段时，允许保留
        # 当前事实/等式推导值；上期是否曾披露不参与本期判断。
        allowed = field in product_fact_fields or counterpart in product_fact_fields
        if not allowed:
            for row in rows or []:
                row[field] = ""
...
def _apply_document_fiscal_context(rows, target_table, document_period_text,
    ...
    table_text = " ".join(str(cell or "") for row in (target_table or [])
                          if isinstance(row, list) for cell in row)
    # “财年中期/FY interim”只有财政年度标签而无日历截止日时，用上一正式
    # 财年截止日推导本期六个月区间。历史只提供财历锚点，不提供金额。
    if re.search(r"財年中期|财年中期|fiscal\s+year\s+interim|FY\s*interim", table_text, re.I):
        lp_ends = []
        for item in last_period_data or []:
            if not isinstance(item, dict):
                continue
            ...
            if match:
                lp_ends.append(datetime.date(*map(int, match.groups())))
        if lp_ends:
            anchor = max(lp_ends)
            ...
            for row in rows or []:
                try:
                    parsed_ends.append(datetime.date.fromisoformat(str(row.get("end_date") or "")))
                except ValueError:
                    pass
            latest_parsed_year = max((value.year for value in parsed_ends), default=None)
            for row in rows or []:
                try:
                    old_end = datetime.date.fromisoformat(str(row.get("end_date") or ""))
                except ValueError:
                    continue
                year_offset = 0 if old_end.year == latest_parsed_year else old_end.year - latest_parsed_year
                row["start_date"] = start.replace(year=start.year + year_offset).isoformat()
                row["end_date"] = end.replace(year=end.year + year_offset).isoformat()
            return
...
def _extract_revenue_rows(table, typ, last_period_data=None, title=""):
    explicit_products = bool(
        re.search(r"產品|产品|商品|服務|服务", str(title or ""), re.I)
        and re.search(r"收入|收益|營業額|营业额|銷售|销售|Revenue", str(title or ""), re.I)
        and re.search(r"構成|构成|分類|分类|類別|类别|劃分|划分|主要由|分析", str(title or ""), re.I)
        and not re.search(r"收入確認主要|收入确认主要|確認主要考慮|确认主要考虑|交易價格|交易价格|完成服務日期|完成服务日期", str(title or ""), re.I)
    )
    """把表类型路由到统一的收入事实行，供主表和候选表共同使用。"""
    if typ in ("收入_行产品", "分部_行产品"):
        return extract_type1(table, last_period_data, explicit_products=explicit_products)
...
def _legacy_get_res_adapter(selected, info_code, reason_arr, notice_date="",
                            last_period_data=None, source_tables=None,
                            document_period_text=""):
    """Temporary adapter around legacy fact materializers.

    It is intentionally kept outside the new pipeline modules.  New disclosure
    decisions must not be added here; concrete structure materializers will
    replace this adapter one at a time.
    """
    res = {"target_res": [], "pipe_meta": {"selected_count": 0, "source_pages": []}}
    target_table = selected.get("target_table") if isinstance(selected, dict) else (selected or [])
    ...
    if page_number is not None:
        res["pipe_meta"]["source_pages"] = [page_number]
    if selected:
        res["pipe_meta"]["selected_count"] = 1

    rows = []
    typ = classify_table(target_table, title, last_period_data, page_lines) if target_table else ""
    if typ:
        if "_含成本段" in typ:
            target_table = _trim_cost_section(target_table)
            typ = typ.replace("_含成本段", "")
        rows = _extract_revenue_rows(target_table, typ, last_period_data, title)
    # Temporary legacy materializer boundary.  The new pipeline owns selection;
    # legacy extraction remains isolated here only until concrete materializers
    # replace each supported table structure.
    hypothesis_sources = list(source_tables or [])
    if isinstance(selected, dict) and selected.get("target_table"):
        hypothesis_sources.append(selected)
    res["pipe_meta"].update({
        "architecture": "evidence_hypothesis_materialization",
        "adapter": "legacy_fact_materializer",
        "source_table_count": len(hypothesis_sources),
    })

    if not rows:
        if not selected:
            reason_arr.append("未选到目标表")
        elif not typ:
    ...
    # 检测币种并注入每条记录
    # 币种与单位一样是稳定输出口径；长公告正文可能列举多种外币，不能让
    # 文档级偶然命中覆盖上一正式期的一致币种。
    currency = _last_period_consensus(last_period_data, "CURRENCY")
    if not currency:
        currency = _detect_currency(target_table)
    if not currency:
        currency = _detect_document_currency(hypothesis_sources, page_number)
    for r in rows:
        if not r.get("currency") and (r.get("_source_currency") or currency):
            r["currency"] = r.get("_source_currency") or currency

    # 检测单位（百万元/千元/元）并注入每条记录
    # 单位是公告级输出口径；历史口径稳定时优先使用。候选表正文可能同时出现
    # “百万元”等叙述性金额，不能让它覆盖整套产品事实的既有呈列单位。
    # 单位是输出口径而非金额事实；同一产品流水线通常延续既有单位。当前表中
    # 还可能混有“百万元”等叙述性金额，因此稳定的上期单位优先，显式表头兜底。
    ...
    if not unit_code:
        ...
        if target_numbers and max(target_numbers) > 1e8:
            unit_code = "001"
    if not unit_code:
        unit_code = _detect_document_unit(hypothesis_sources, page_number)
    if not unit_code:
        unit_code = _detect_unit(target_table, currency)
    for r in rows:
        if not r.get("unit") and (r.get("_source_unit") or unit_code):
            r["unit"] = r.get("_source_unit") or unit_code

    # 分部表：从原始（未截断）全表中提取 GROSS_PROFIT 和 MBCOST
    # Cost and gross-profit facts are merged later from all source tables after the
    # primary revenue hypothesis is fixed.  The entry table has no special merge path.

    _apply_document_fiscal_context(rows, target_table, document_period_text, last_period_data)

    # 后处理：过滤明显的P&L噪声行（短标签+精确匹配PL模式+不在LP中）
    explicit_product_table = bool(
        re.search(r"產品|产品|商品|服務|服务", str(title or ""), re.I)
        and re.search(r"收入|收益|營業額|营业额|銷售|销售|Revenue", str(title or ""), re.I)
        and re.search(r"構成|构成|分類|分类|類別|类别|劃分|划分|主要由|分析", str(title or ""), re.I)
        and not re.search(r"收入確認主要|收入确认主要|確認主要考慮|确认主要考虑|交易價格|交易价格|完成服務日期|完成服务日期", str(title or ""), re.I)
    )
    if len(rows) > 1 and not explicit_product_table:
    ...
    if hypothesis_sources and rows:
        metric_facts = _collect_metric_facts(hypothesis_sources, rows)
        res["pipe_meta"]["source_table_count"] = len(hypothesis_sources)
...
def get_res(selected, info_code, reason_arr, notice_date="", last_period_data=None,
    ...
    """Public compatibility entry while the new pipeline components are wired."""
    return _legacy_get_res_adapter(
        selected,
        info_code,
        reason_arr,
...
def extract_type1(table, last_period_data=None, explicit_products=False):
    """从行产品表中提取，用上期产品名过滤"""
    # 在表头/正文切分前保存无金额父行到破折号子项的层级关系，否则父行会被
    # 判作表头，正文 extractor 无法恢复当前公告披露的完整名称路径。
    child_parent_map = {}
    structural_parent = ""
    for source_row in table or []:
        if not isinstance(source_row, list) or not source_row:
            continue
        source_name = fullwidth_to_halfwidth(str(source_row[0] or "").strip())
        has_amount = any(_amount_value(cell) is not None for cell in source_row[1:])
        if source_name and not has_amount and not re.search(r"截至|年度|期間|期间|人民幣|人民币|美元", source_name):
            structural_parent = _clean_product_name(source_name.rstrip("：:"))
        elif re.match(r"^[-\–—]{1,3}", source_name) and structural_parent:
            child = _clean_product_name(source_name)
            if child:
                child_parent_map[child] = structural_parent

    has_named_grand_total = any(
        isinstance(source_row, list) and source_row
        and re.search(r"合併收入|合并收入|總收入|总收入|總計|总计", str(source_row[0] or ""))
        for source_row in table or []
    )
    ...
    if len(block_starts) >= 2:
        ...
        for pos, start in enumerate(block_starts):
            ...
            if len(block) >= 2:
                merged.extend(extract_type1(block, last_period_data, explicit_products))
        deduped = {}
        for item in merged:
    ...
    for row in rows_to_scan:
        ...
        # “其中:”是层级开始标记，不代表该行不是产品；是否保留父项由后面的
        # 金额闭合关系判断。
        if re.match(r'其中[：:\s]', name):
            name = re.sub(r'^其中[：:\s]+', '', name).strip()
        ...
        if re.match(r"^(合[计計]|總[计計]|总[计計]|小[计計]|總額|总额|"
                     ...
                     r"總收入|总收入|總收益|总收益|合併收入|合并收入|綜合收入|综合收入|"
                     r"(?:營業|营业|主營業務|主营业务)?(?:收入|收益).*(?:合計|合计|總計|总计)$)", name):
            name = "合计"
        if not name:
            continue
        ...
        if is_child and (child_parent_map.get(name) or last_parent) not in ("", "合计"):
            name = (child_parent_map.get(name) or last_parent) + "--" + name
    ...
    if has_named_grand_total:
        raw = [
            ...
            if not (
                re.search(r"(?:集團|集团)?合[計计]$|小[計计]$", name)
                and name not in lp_names
            )
        ]

    # 当前表内的金额闭合比历史产品集合更可靠：若一行金额等于其后至少两行
    # 连续明细之和，该行是父级小计，输出子项而不是父项。要求所有可比期间
    # 同时闭合，避免偶然数值相等。
    subtotal_indexes = set()
    numeric_vectors = []
    for _name, vals in raw:
        numeric_vectors.append([_amount_value(value) for _sd, _ed, value in vals])
    for parent_index, parent_values in enumerate(numeric_vectors):
        if not parent_values or any(value is None for value in parent_values):
            continue
        running = [0.0] * len(parent_values)
        child_count = 0
        for child_index in range(parent_index + 1, len(numeric_vectors)):
            child_values = numeric_vectors[child_index]
            if len(child_values) != len(parent_values) or any(value is None for value in child_values):
                break
            child_count += 1
            running = [left + right for left, right in zip(running, child_values)]
            if child_count >= 2 and all(
                abs(actual - expected) <= max(1.0, abs(expected) * 1e-8)
                for actual, expected in zip(running, parent_values)
            ):
                subtotal_indexes.add(parent_index)
                break
            if all(abs(actual) > abs(expected) + 1 for actual, expected in zip(running, parent_values)):
                break
    if subtotal_indexes:
        lp_identity_names = {
            fullwidth_to_halfwidth(str(item.get("PRODUCTNAME", "") or "").strip())
        ...
        raw = [
            entry for index, entry in enumerate(raw)
            if index not in subtotal_indexes or entry[0] in lp_identity_names
        ]
    ...
    if lp_names and not explicit_products:
        kept = []
        removed = []
    ...
    raw_name_counts = {}
    for raw_name, _raw_vals in raw:
        raw_name_counts[raw_name] = raw_name_counts.get(raw_name, 0) + 1
    repeated_product_count = sum(
        1 for raw_name, count in raw_name_counts.items()
        if raw_name != "合计" and count >= 2
    )
    section_header_count = sum(
        1 for source_row in table
        if isinstance(source_row, list) and source_row
        and str(source_row[0] or "").strip()
        and not any(_amount_value(cell) is not None for cell in source_row[1:])
    )
    has_explicit_grand_total = any(
        isinstance(source_row, list) and source_row
        and re.search(r"總計|总计|合計|合计", str(source_row[0] or ""))
        for source_row in table
    )
    grouped_repeated_products = (
        repeated_product_count >= 2
        and section_header_count >= 2
        and has_explicit_grand_total
    )

    # 4. 格式化输出 + 值级别去重 + 自动补合计
    out = []
    for name, vals in raw:
        # 产品名以当前公告原文为准。上期名称只参与识别，不覆盖本期披露名称。
        for sd, ed, v in vals:
    ...
    if out:
        ...
        for key, entries in groups.items():
            ...
            elif grouped_repeated_products and key[0] != "合计":
                values = [_to_num(entry["mbrevenue"]) for entry in entries]
                values = [value for value in values if value is not None]
                if values:
                    total = sum(values)
                    merged = dict(entries[0])
                    merged["mbrevenue"] = str(int(total) if float(total).is_integer() else total)
    ...
    # 合计只能来自当前公告明确披露的合计行。历史产品结构不能要求本期必须
    # 有合计，更不能用本期叶子相加制造一个公告未披露的金额。
    if has_explicit_grand_total and not any(e["product_name"] == "合计" for e in out):
        period_sums = {}
        other_names = set()
        for entry in out:
            if entry["product_name"] == "合计":
                continue
            value = _to_num(entry["mbrevenue"])
            if value is None:
                continue
            key = (entry["start_date"], entry["end_date"])
            period_sums[key] = period_sums.get(key, 0) + value
            other_names.add(entry["product_name"])
        if len(other_names) >= 2:
            for (sd, ed), total in period_sums.items():
                out.append({
                    "product_name": "合计",
                    "mbrevenue": str(int(total) if float(total).is_integer() else total),
                    "start_date": sd, "end_date": ed,
                    "year": sd[:4] if sd else ed[:4] if ed else "",
                })
    return out
    ...
    if not revenue_row:
        ...
        for r in body:
            ...
            if _REV_GEN.search(lab) and not re.search(r"成本|费用|費用|開支|开支|税|稅|利息", lab):
                ...
                if valid_cols > 0:
                    # 同表同时披露管理层“基本/经调整”口径与正式列账/法定口径
                    # 时，产品收入采用正式报告口径；这属于 metric basis，不是
                    # 用历史金额猜哪一行。
                    official_basis = bool(re.search(
                        r"列賬基準|列账基准|法定|報告基準|报告基准|reported",
                        lab, re.I,
                    ))
                    adjusted_basis = bool(re.search(
                        r"基本|基礎|基础|經調整|经调整|underlying|adjusted",
                        lab, re.I,
                    ))
                    is_sub = bool(_REV_SUBITEM.search(lab))
                    score = (
                        0 if official_basis else 2 if adjusted_basis else 1,
                        0 if not is_sub else 1,
                        len(lab),
                        -valid_cols,
                    )
...
def _map_product_name_to_lp(name, last_period_data=None):
    """保留当前公告产品名；历史名称不得改写本期正式披露。"""
    return fullwidth_to_halfwidth(str(name or "").strip())
...
def extract_type3(table, last_period_data=None):
    ...
    # 收益桥接损益表：总收益之前同时列示正、负收入构成和净额小计。用历史
    # 身份识别当前叶子，但必须由当前叶子金额自行闭合到当前总收益才采用。
    total_revenue_index = next((
        index for index, row in enumerate(body)
        if isinstance(row, list) and row
        and re.fullmatch(r"收益|收入|營業收入|营业收入|總收益|总收益|總收入|总收入", str(row[0] or "").strip())
        and any(_amount_value(cell) is not None for cell in row[1:])
    ), None)
    if total_revenue_index is not None and lp_names:
        bridge_rows = []
        for row in body[:total_revenue_index]:
            if not isinstance(row, list) or len(row) < 2:
                continue
            label = fullwidth_to_halfwidth(str(row[0] or "").strip())
            if re.search(r"淨額|净额", label):
                continue
            if any(_lp_name_in_cell(lp_name, label) for lp_name in lp_names):
                bridge_rows.append(row)
        if len(bridge_rows) >= 2:
            bridge_output = []
            for row in bridge_rows:
                bridge_output.extend(_format_type3_output(
                    row, periods, _clean_product_name(str(row[0] or ""))
                ))
            total_output = _format_type3_output(body[total_revenue_index], periods, "合计")
            leaves_by_period = {}
            for item in bridge_output:
                key = (item.get("start_date"), item.get("end_date"))
                value = _amount_value(item.get("mbrevenue"))
                if value is not None:
                    leaves_by_period[key] = leaves_by_period.get(key, 0.0) + value
            closed = bool(total_output) and all(
                _amount_value(item.get("mbrevenue")) is not None
                and abs(
                    leaves_by_period.get((item.get("start_date"), item.get("end_date")), float("inf"))
                    - _amount_value(item.get("mbrevenue"))
                ) <= max(1.0, abs(_amount_value(item.get("mbrevenue"))) * 1e-8)
                for item in total_output
            )
            if closed:
                return bridge_output + total_output
